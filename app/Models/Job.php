...
class Job
{
    ...
    public function addArticle($title, $company, $description, $location, $status, $dateCreated, $imagePath)
    {
        ...
        $target='C:/xampp/htdocs/MVC/public/uploads/';
...
public function search($key,$company,$location)
{
    $stmt = "SELECT * FROM jobs where title like '%$key%' and company like '%$company%' and location like '%$location%'";
    $result = $this->conn->query($stmt);
    $getalph = [];
    while ($row = $result->fetch_assoc()) {
        $getalph[] = $row;
...
{
    $stmt = "SELECT * FROM jobs where id=$id";
    $result = $this->conn->query($stmt);
    $getalph = [];
    while ($row = $result->fetch_assoc()) {
        $getalph[] = $row;
